...
const columns = computed(() => [
  ...
  { label: t("cluster.nodeName", "节点名称"), prop: "name", slot: "name", minWidth: 160 },
  { label: t("cluster.nodeAddr", "API 地址"), prop: "addr", slot: "addr", minWidth: 200 },
  { label: t("cluster.status", "节点状态"), prop: "status", slot: "status", minWidth: 140 },
  { label: t("cluster.lastPing", "最后心跳时间"), prop: "last_ping", slot: "last_ping", minWidth: 170 },
  { label: t("cluster.remark", "备注"), prop: "remark", minWidth: 140 },
  { label: t("rule.operation", "操作"), slot: "operation", fixed: "right", minWidth: 300 }
]);
...
async function fetchData() {
  ...
  try {
    ...
    if (res.code === 0 && res.data) {
      ...
      message(res.message || t("cluster.pingError", "获取节点列表失败"), { type: "error" });
    }
  } finally {
    loading.value = false;
  }
  ...
  FormRef.validate(async (valid: boolean) => {
    if (valid) {
      ...
      try {
        ...
        if (res.code === 0) {
          message(res.message || t("cluster.success", "成功"), { type: "success" });
          showView.value = "list";
          fetchData();
        } else {
          message(res.message || t("rule.submitFailed", "提交失败"), { type: "error" });
        }
      } finally {
        saving.value = false;
      }
...
async function handleDelete(row: ClusterNodeItem) {
  ...
  if (res.code === 0) {
    ...
    message(res.message || t("rule.deleteFailed", "删除失败"), { type: "error" });
  }
}
...
async function handlePing(row: ClusterNodeItem) {
  ...
  if (res.code === 0) {
    message(t("cluster.pingSuccess", "节点通信及鉴权正常"), { type: "success" });
  } else {
    ...
    if (errStr === "auth_failed") {
      ...
      errStr = errStr ? `${t("cluster.pingError", "节点离线或通信异常")} (${errStr})` : t("cluster.pingError", "节点离线或通信异常");
    }
...
async function handleSync(row: ClusterNodeItem) {
  ...
  if (res.code === 0) {
    message(res.message || t("cluster.pushSuccess", "配置下发成功"), { type: "success" });
  } else {
    message(res.message || t("cluster.pushError", "配置下发失败"), { type: "error" });
  }
  fetchData();
}
...
async function handleSyncAll() {
  ...
  if (res.code === 0) {
    message(res.message || t("cluster.syncAllSuccess", "同步配置指令已触发"), { type: "success" });
  } else {
    message(res.message || t("cluster.syncAllError", "同步配置失败"), { type: "error" });
  }
  fetchData();
}
...
<template>
  <div class="main">
    ...
    <div v-if="showView === 'list'">
      ...
      >
        <el-form-item :label="t('cluster.nodeName', '节点名称') + '：'" prop="name">
          <el-input
            v-model="searchForm.name"
            :placeholder="t('cluster.searchNamePlaceholder', '搜索节点名称或 API 地址')"
            clearable
      ...
      >
        ...
        <template v-slot="{ size, dynamicColumns }">
          ...
          <div
            v-if="selectedNum > 0"
            class="bg-(--el-color-primary-light-9) text-(--el-color-primary) border border-(--el-color-primary-light-7) px-4 py-2 rounded-lg text-sm mb-3 flex items-center justify-between"
          >
            <span>
              {{ t("rule.selected", "已选择") }} {{ selectedNum }} {{ t("rule.items", "项") }}
            </span>
            <el-popconfirm
              :title="t('cluster.batchDeleteConfirm', '确认批量删除选中的集群节点？')"
              @confirm="handleBatchDelete"
            >
              <template #reference>
                <el-button type="danger" size="small" link :icon="useRenderIcon(Delete)">
          ...
          >
            <template #name="{ row }">
              <span class="font-bold text-(--el-text-color-primary)">{{ row.name }}</span>
            </template>

            <template #addr="{ row }">
              <code class="font-mono text-xs text-(--el-color-primary)">{{ row.addr }}</code>
            </template>

            <template #status="{ row }">
              <el-tag v-if="row.status === 1" type="success" effect="dark" class="font-mono">
                {{ t("cluster.online", "🟢 在线") }}
              </el-tag>
              <el-tag v-else-if="row.status === 2" type="warning" effect="dark" class="font-mono">
            ...
            <template #operation="{ row }">
              ...
              <el-popconfirm
                :title="t('cluster.confirmDelete', '是否确认删除该集群节点配置?')"
