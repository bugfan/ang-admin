...
const acmeServers = [
  {
    label: "Let's Encrypt (生产环境 - 推荐)",
    value: "https://acme-v02.api.letsencrypt.org/directory"
  },
  {
    label: "Let's Encrypt (Staging 测试环境 - 无速率限制)",
    value: "https://acme-staging-v02.api.letsencrypt.org/directory"
  },
  {
    label: "ZeroSSL (需配置 EAB 凭据)",
    value: "https://acme.zerossl.com/v2/DV90"
  },
  {
    label: "Google Trust Services (GTS - 需 EAB 凭据)",
    value: "https://dv.acme-v02.api.pki.goog/directory"
  },
  {
    label: "BuyPass Go SSL (免 EAB, 180天单域名)",
...
const acmeAccounts = [
  ...
  },
  {
    ...
    envKeys: [
      {
        key: "DNSPOD_API_KEY",
        label: "DNSPod Token (ID,Token)",
        placeholder: "格式: ID,Token (例如: 12345,6789abcdef...)"
      }
    ],
  ...
  },
  {
    ...
    envKeys: [
      {
        key: "HUAWEICLOUD_ACCESS_KEY_ID",
        label: "Access Key (AK)",
        placeholder: "例如: AK..."
      },
      {
        key: "HUAWEICLOUD_SECRET_ACCESS_KEY",
...
const formRules = reactive({
  name: [
    {
      required: true,
      message: () => t("acmeAccount.name") + " " + t("acmeAccount.required"),
      trigger: "blur"
    }
  ]
});
  ...
  >
    <el-row :gutter="20">
      ...
      <re-col :value="24" :xs="24" :sm="24">
        <div
          class="font-bold text-sm text-(--el-text-color-primary) border-b border-(--el-border-color-lighter) pb-2 mb-4 flex items-center gap-1.5"
        >
      ...
      <template v-if="showEabSection">
        <re-col :value="24" :xs="24" :sm="24">
          <div
            class="font-bold text-sm text-(--el-text-color-primary) border-b border-(--el-border-color-lighter) pb-2 my-4 flex items-center gap-1.5"
          >
        ...
        <re-col :value="24" :xs="24" :sm="24">
          <div
            class="bg-(--el-fill-color-light) p-3 rounded-xl mb-3 text-xs text-(--el-text-color-secondary) border border-(--el-border-color-lighter) flex items-center gap-2"
          >
            <IconifyIconOffline
              icon="ri:information-line"
              class="text-amber-500 shrink-0 text-base"
            />
      ...
      <re-col :value="24" :xs="24" :sm="24">
        ...
        >
          <IconifyIconOffline
            icon="ri:information-line"
            class="text-blue-500 shrink-0 text-base"
          />
