...
func init() {
	rest.Register(&models.UdpProxy{}, &udpHandler{}, rest.RouteTypeALL, nil, "udp-proxy")
}
...
func (t *udpHandler) Before(g *gin.Context, x *xorm.Engine) bool {
	...
	if method == http.MethodPost || method == http.MethodPut || method == http.MethodPatch {
		...
		// Check for duplicate port in udp_proxy
		targetId := t.Id
		if targetId == 0 {
		...
		session := models.GetEngine().Table("udp_proxy").Where("port = ?", portStr)
		if targetId > 0 {
...
func (t *udpHandler) After(g *gin.Context, x *xorm.Engine, args ...interface{}) {
	if g.Request.Method == http.MethodPost || g.Request.Method == http.MethodPut || g.Request.Method == http.MethodPatch || g.Request.Method == http.MethodDelete {
		service.SyncUDPToCluster()
	}
}

func (t *udpHandler) List(c *gin.Context) {
	var list []models.UdpProxy
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"code": 1, "message": "查询 UDP 代理列表失败: " + err.Error()})
		return
	}

	resList := make([]udpHandler, 0, len(list))
	for _, item := range list {
		resList = append(resList, udpHandler{
			Id:              item.Id,
			Name:            item.Name,
